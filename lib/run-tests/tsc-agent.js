"use strict";

const fs = require("fs");

const NodeAgent = require("eshost/lib/agents/node");

const transpile = require("./transpile");

const CORE_JS = require.resolve("core-js-bundle");

const POLYFILLS = `[
  'Function("this.globalThis = this;")()',
  fs.readFileSync(${JSON.stringify(CORE_JS)}, "utf8"),
]`;

module.exports = class TscAgent extends NodeAgent {

  static runtime = fs
    .readFileSync(__dirname + "/tsc-agent-runtime.js", "utf8")
    .replace(/\$POLYFILLS/g, POLYFILLS);

  constructor(options) {
    super({ shortName: "$262", ...options });
    this.transpiler = options.transpiler;
  }

  compile(code) {
//    console.log(`code = ${JSON.stringify(code)}`);
    code = super.compile(code);

    code = code.replace(
      /(?<=var eshostContext = vm\.createContext\(\{)/,
      `exports: exports,`
    );
    code = code.replace(
      /(?=vm\.runInESHostContext\()/,
      `
        var fs=require("fs");
        var polyfills = ${POLYFILLS};
        for (var i = 0; i < polyfills.length; i++) {
          vm.runInESHostContext(polyfills[i]);
        }
      `
    );

//    console.log(`### this.transpiler = ${this.transpiler}`);
    code = transpile.transpile(code, {}, this.transpiler);
//    console.log(`Returning ${code}`);
    return code.code;
  }
};
