...
exports.runTest = async function (test) {
  ...
  console.log(`file = ${file} isModule = ${isModule}`);
  let transpileTime;

//  const cacheKey = sha1(JSON.stringify({ attrs, file }));

  try {
    const { code, time } = transpile.transpile(contents, {
  ...
  console.log(`path: ${file} contents: ${contents} attrs = ${JSON.stringify(attrs)}`);
...
*/
  ...
  try {
    ...
    const runtime = timeEnd - timeBegin;

    if (result.error) {
      ret = { result: "runtime error", error: result.error, transpileTime, runtime };
    } else {
      ret = { result: "success", output: result.stdout, transpileTime, runtime };
    }
  } catch (error) {
