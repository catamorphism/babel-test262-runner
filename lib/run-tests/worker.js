...
//    console.log(`${Date.now()} worker got result: ${file}`);
    ...
    if (result.error) {
      ret = { result: "runtime error", error: result.error, time: contents.time };
    } else {
      ret = { result: "success", output: result.stdout, time: contents.time };
    }
  } catch (error) {
    agent.stop(); // kill process avoid 100% cpu usage

    ret = { result: "timeout error", error, time: contents.time };
  }
