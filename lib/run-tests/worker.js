...
const TscAgent = require("./tsc-agent");
...
//  console.log(`~~~~ transpiler = ${opts.transpiler}`);
  agent = new TscAgent(opts);
...
//  const cacheKey = sha1(JSON.stringify({ attrs, file }));
 ...
 // console.log(`path: ${file} contents: ${contents}`);
...
*/
  ...
  let ret;
  try {
//    console.log(`${Date.now()} worker begin: ${file}`);
//    console.log(`%%%% ${JSON.stringify(testObject)}`);
    result = await Promise.race([agent.evalScript(testObject), timeout(file)]);
//    console.log(`${Date.now()} worker got result: ${file}`);

    if (result.error) {
      ret = { result: "runtime error", error: result.error, time: contents.transpiledTime };
    } else {
      ret = { result: "success", output: result.stdout, time: contents.transpiledTime };
    }
  } catch (error) {
    agent.stop(); // kill process avoid 100% cpu usage

    ret = { result: "timeout error", error, time: contents.transpiledTime };
  }

//  console.log(`${Date.now()} worker end: ${file} ${ret}`);
