...
const timingMap = require('./timing-map');
...
//      console.log(`------- ${innerTests.length}`);
      ...
      for (const innerTest of innerTests) {
        ...
        (async test => {
          ...
          const transpileTime = actual.transpileTime;
          const runtime = actual.runtime;
          if (actual.result === expected) {
            passed++;
            if (transpileTime)
              totalTranspileTime += transpileTime;
            if (runtime)
              totalRuntime += runtime;
            tap.pass(file, `(${expected}) (transpile time: ${formatTime(transpileTime)} ms) (runtime: ${formatTime(runtime)} ms}`);
          } else {
            tap.fail(
              file,
  ...
  if (run > 0) {
    ...
    tap.diag(`Average runtime (successes only): ${formatTime(totalRuntime/passed)} ms`);
  }
  ...
  console.log(`total time: ${formatTime(timeEnd - timeBegin)} ms`);
  process.exit(0);
}
main().catch(error => {
  process.exitCode = 1;
